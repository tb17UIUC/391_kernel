...
int32_t rtc_write(int32_t fd, const int32_t* buf, int32_t nbytes) {
    ...
    if (*buf < RTC_MIN || *buf > RTC_MAX) {
        ...
        terminals[sched_term].counter_freqRate = (RTC_MAX / (2 * frequency));   // Calculating interval to virtaully set frequency
        
    }

    // Calculate rate value (power of 2); 2^(rate value) = freq
    terminals[sched_term].rtc_counter = 0;
